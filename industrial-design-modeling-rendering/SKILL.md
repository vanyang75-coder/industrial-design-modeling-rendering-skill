...
4. Use the problem-driven library when the user describes a symptom or practical design problem, but treat it as a taxonomy unless detailed rules are present.
5. Use software-specific folders for detailed examples and product-type analogies.
6. Use rule indexes for category navigation, not as proof that every claimed rule exists in full detail.
7. Avoid backup, archive, and virtual-environment folders unless the user asks for historical recovery or the current sources are missing.
8. If no good match appears, say that the local knowledge base did not contain a close match and provide a reasoned workflow as an inference.
